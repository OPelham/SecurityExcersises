...
public class Cryptanalysis {
	...
	public static void frequencyAnalysis(String cryptoText) {
		...
		ArrayList<Integer> orderedPos = new ArrayList<>();
		//init freqlist size
		for (int i=0; i<Cypher.alphabet.length(); i++) {
			frequecyList.add(0);
		}
		...
		int maxPos = -1;
		for (int j=0; j<20; j++) { //try 20 most common chars
			for (int i=0; i<frequecyList.size(); i++) {
				if(frequecyList.get(i) > maxPos && (!(orderedPos.contains(i)))) {
					maxPos = i;
				}
			}
			orderedPos.add(maxPos);
		}
		
		//now try first most common as a and test if not try next and test etc
		//calculate shift from most frequent pos to pos of e
